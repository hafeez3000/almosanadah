...
<form name="roomselput" action="resroomamendsela.php"  method="post" onSubmit="return fun2(this)">
...
$_SESSION["madselrcb"] = isset($_POST["madselrcb"]) ? $_POST["madselrcb"] : '';

$mad_arr_room_id = isset($_POST["madselrcb"]) ? $_POST["madselrcb"] : '';
...
for($i=0; $i<count((array)$mad_arr_room_id); $i++){
...
for($we=0; $we<count((array)$a_weekends); $we++){
...
echo ucfirst((string)$mad_array_sel_meals[$ri][$me]) ;
$sr_meals="0";
$compare_meal_string = isset($mad_array_sel_meals[$ri][$me]) ? trim($mad_array_sel_meals[$ri][$me]) : '';

if($ss_breakfast==$compare_meal_string){
  if(trim((string)$s_breakfast)=="Included"){
  $sr_meals="INC";
  }
  else if(trim((string)$s_breakfast)=="Not Available"){
  $sr_meals="NA";
  }
  else{
  $sr_meals=$s_breakfast;
  $tot_meals = $tot_meals+(int)$s_breakfast;
  }
  }


if($ss_halfboard==$compare_meal_string){
if(trim((string)$s_halfboard)=="Included"){
$sr_meals="INC";
}
else if(trim((string)$s_halfboard)=="Not Available"){
$sr_meals="NA";
}
else{
$sr_meals=$s_halfboard;
$tot_meals = $tot_meals+(int)$s_halfboard;
}
}

if($ss_fullboard==$compare_meal_string){
if(trim((string)$s_fullboard)=="Included" ){
$sr_meals="INC";
}
else if(trim((string)$s_fullboard)=="Not Available"){
$sr_meals="NA";
}
else{
//echo $s_fullboard;
$sr_meals=$s_fullboard;
$tot_meals = $tot_meals+(int)$s_fullboard;
}
}

if($ss_sahoor==$compare_meal_string){
if(trim((string)$s_sahoor)=="Included" ){
$sr_meals="INC";
}
else if(trim((string)$s_sahoor)=="Not Available"){
$sr_meals="NA";
}
else{

$sr_meals=$s_sahoor;
$tot_meals = $tot_meals+(int)$s_sahoor;
}
}

if($ss_iftar==$compare_meal_string){
if(trim((string)$s_iftar)=="Included" ){
$sr_meals="INC";
}
else if (trim((string)$s_iftar)=="Not Available"){
$sr_meals="NA";
}
else{
//echo $s_iftar;
$sr_meals=$s_iftar;
$tot_meals = $tot_meals+(int)$s_iftar;
}
}
...
if($mad_array_p_n_rate[$ri] || $mad_array_p_rate[$ri]){
$sr_meals = trim((string)$mad_array_sel_mealsval[$ri]);
echo "<br>";
...
'<? for($m=0; $m<count((array)$mad_array_sel_meals[$ri]); $m++){?>'
...
'<? for($mer=0; $mer<count((array)$mad_array_sel_meals[$ri]) ; $mer++){ ?>'
...
'<? for($mern=0; $mern<count((array)$mad_array_sel_meals[$ri]) ; $mern++){ ?>'
...
//echo "<tr><td colspan=\"7\"><div align=\"right\"><font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\">GT.Net:<br><input type=\"text\" id=\"mad_hotntot$ri\" name=\"mad_hotntot$ri\" size=\"4\"  style=\"text-align:right;\" value='$mad_hot_ntot' readonly></font></div></td><td colspan=\"7\"><div align=\"right\"><font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\">GT.Sell:<br><input type=\"text\" id=\"mad_hottot$ri\" name=\"mad_hottot$ri\" size=\"4\"  style=\"text-align:right;\" value='$mad_hot_tot' readonly></font></div></td></tr>";
...
'<? for($gtot=0; $gtot<count((array)$mad_array_room_id); $gtot++){?>'
...
'<? for($gntot=0; $gntot<count((array)$mad_array_room_id); $gntot++){?>'
...
<tr><td colspan="2" align="center"><input type="submit" value="Amend Selected Room"></td></tr>
...
<script>
    function fun2(theForm) {
      ...
      var numOfNights = <? echo $madnights1; ?>;
      ...
      for (i = 0; i < numOfNights; i++) {
        ...
        if (isNaN(document.getElementById('mad_putnrate' + fN).value) || document.getElementById('mad_putnrate' + fN).value == "" || document.getElementById('mad_putnrate' + fN).value == "0") {
          alert("Please check the Net Rate Once Again");
          allFieldsFilled = false;
          break;
        }

        if (isNaN(document.getElementById('mad_putrate' + fN).value) || document.getElementById('mad_putrate' + fN).value == "" || document.getElementById('mad_putrate' + fN).value == "0") {
