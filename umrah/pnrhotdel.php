...
$hsresult1 = pg_query($conn, $hsquery1);
...
$hsresult = pg_query($conn, $hsquery);
if (!$hsresult) {
...
$hnresult = pg_query($conn, $hnquery);
if (!$hnresult) {
  echo "An error occured.\n";
  exit;
}
...
pg_query($conn, "update sales_main set sales_hotels='f' where ocode='$s_pnr'");
}
...
$transresult1 = pg_query($conn, $transquery1);
...
$transresult = pg_query($conn, $transquery);
if (!$transresult) {
  echo "An error occured.\n";
  exit;
}
...
pg_query($conn, "update sales_main set sales_trans='f' where ocode='$s_pnr'");
}
...
$visaresult1 = pg_query($conn, $visaquery1);
...
$visaresult = pg_query($conn, $visaquery);
if (!$visaresult) {
  echo "An error occured.\n";
  exit;
}
...
pg_query($conn, "update sales_main set sales_visa='f' where ocode='$s_pnr'");
}
...
$extraresult1 = pg_query($conn, $extraquery1);
...
$extraresult = pg_query($conn, $extraquery);
if (!$extraresult) {
  echo "An error occured.\n";
  exit;
}
...
pg_query($conn, "update sales_main set sales_others='f' where ocode='$s_pnr'");
}
