...
$main_sel_checkr = pg_query($conn, $q_main_sel_checkr);

$rows_main = pg_num_rows($main_sel_checkr);
$rows_main_check = $rows_main;
if($rows_main==0){ 
...
$q_main_sel ="select main_sno,ocode,user_sno,is_online,user_id,guest_title,guest_name,guest_nationality, guest_telno,guest_notes,flight_det,order_date,option_date,booking_status,cus_title,cus_name,cus_company_name,cus_country,sales_hotels,sales_trans,sales_visa,sales_others,group_code from sales_main where ocode='$s_pnr'";
...
$s_group_code = $rows_main["group_code"];
$s_guest_nationality = $rows_main["guest_nationality"];
...
if($rows_main["is_online"]) { echo $s_online = "<font color=\"red\"> (ONLINE) </font>" ; }
...
if($s_group_code) {
    echo "<br>";
    echo "Group Code: ". $s_group_code;
}

echo "<br>";
echo "Nationality: ". $s_guest_nationality ;
echo "<br>";
...
echo "<a href=\"changeopdatep.php?spnr=$s_pnr\" onclick=\"return can_chk();\">";
echo date('d-M-Y H:i', strtotime($s_option_date)) ;
echo "</a>";
}
...
if($amd_dis==1){ 
echo "<tr><td colspan=\"4\" align=\"center\" valign=\"middle\"><font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\"> Amend Guest Det  |  ";
}
else {
...
echo "  |  <a href=\"printitinerary.php?spnr=$s_pnr\" target=\"printitinerary\" onClick=\"window.open('', 'printitinerary','width=775,height=450,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()\" >Print PNR Itinerary</a>";
...
if($amd_dis==1){
echo "<tr><td colspan=\"4\" align=\"center\"><font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\" >Add New Hotel | </FONT>";
}
else{
...
if($amd_dis==1){
echo "<font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\" style=\"BACKGROUND-COLOR: #DFDFDF\"> Add New Trans</font>";
}
...
echo " | <a href=\"pnraddnewvisa.php?spnr=$s_pnr\" onclick=\"return can_chk();\">Add New Visa</a> | ";
}

//echo "</font></td><td colspan=\"1\" align=\"center\">";

if($amd_dis==1) {
echo "<font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\" style=\"BACKGROUND-COLOR: #DFDFDF\">Add New Extras";
echo "</font></td></tr>";
}
else{
...
<br><table width="100%" border="1" cellpadding="1" cellspacing="0">
	
	<?php if($show_upload){ ?>
	<tr >
		<td bgcolor="#CCCCCC"><div align="center"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Customer Voucher & Payments</b>
		
		<a href="customeruploads.php?spnr=<? echo $s_pnr ?>" target="customeruploads" onClick="window.open('', 'customeruploads','width=700,height=380,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()" >Upload</a>

		
	</font></div></td>
			<td bgcolor="#CCCCCC"><div align="center"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Supplier Confirmation & Payments</b> <a href="supplieruploads.php?spnr=<? echo $s_pnr ?>" target="supplieruploads" onClick="window.open('', 'supplieruploads','width=700,height=380,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()" >Upload</a></font></div></td>
	...
	<tr><td>
	<?php
$q_cus_gal ="select  cussupgid,  ocode,   image_path,   is_cus  from cussup_gallery where is_cus=1 and ocode='$s_pnr'";

$cus_gal = pg_query($conn, $q_cus_gal);

$rows_cus_gal = pg_num_rows($cus_gal);
...
function popUp(page, name, details) {
  ...
  return false;
	...
	<?
while ($rows_cus_gal = pg_fetch_array($cus_gal)){

$im_src = $rows_cus_gal["image_path"];

$s_cussupgid = $rows_cus_gal["cussupgid"];
?>

<a href="customerupload.php?sid=<?php echo $s_cussupgid ?>&spnr=<?php echo $s_pnr ?>&img=<?php echo $im_src ?>" target="customerupload" onClick="window.open('', 'customerupload','width=700,height=380,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()" ><img src='<?php echo $uploaddir.$im_src ?>' width="100" height="100"  ></a>
	...
	<?php
$q_cus_gal ="select  cussupgid,  ocode,   image_path,   is_cus  from cussup_gallery where is_cus=0 and ocode='$s_pnr'";
