...
<html>
<head><title>Online Hotel Summary</title>
<script src="onlineclienthint.js"></script> 
</head>
<body>
<center>
...
echo "<font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\">Online <b>" . $h_row["hotel_name"] . " - Summary Between " . date('d-M-Y', strtotime($sfdate)) ." and ". date('d-M-Y', strtotime($stdate)) .
"</b></font>"; 
}
...
$query  = "SELECT ocode,cin, cout, no_rooms, guest_occ_status, cus_paid from sales_hotels where cout between  date '$tod' - integer '45'  and '$tod' and cout > '$fromd' and ocode!='NC' and hotel_id='$hotelid' and booking_status!='Cancelled' and is_online=1 or  cin  between '$fromd' and '$tod' and ocode!='NC' and hotel_id='$hotelid' and booking_status!='Cancelled' and is_online=1 or   cin between date '$tod' - integer '45' and '$tod' and  cout > '$tod' and ocode!='NC' and hotel_id='$hotelid' and booking_status!='Cancelled' and is_online=1 order by cin";
...
$p=0;
for ($ja=0; $ja < $df ; $ja++) {
$querya = "select sum(avialibility) as allotments from rates$hotelid where rate_date between date '$fromd' + integer '$ja'  and date '$fromd' + integer '$ja'  and room_id like '$hotelid%' ";
...
var from_d = '<? echo $sfdate; ?>';
var to_d = '<? echo $stdate; ?>';
