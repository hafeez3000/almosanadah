...
$result_main_seq = pg_query($conn, $query_main_seq);
...
$sqlinsmain = "insert into sales_main ( main_sno,ocode,user_sno,user_id,guest_title,guest_name,guest_nationality,guest_telno,flight_det,guest_notes,order_date,option_date,booking_status,cus_account_code,cus_title,cus_name,cus_company_name,cus_country,cus_contact,cus_email,agent_notes,sales_hotels,sales_trans,sales_visa,sales_others,group_code) values( $main_seq,'$pnr',$suser_sno,'$suserid','$s_gtitle','$s_gname', '$s_gnationality', '$s_contactno','$s_flightdet','$s_guestnotes','now','$optiondate','On Request','$agent_acc_code','$s_cptitle','$s_cpname','$agent_c_name','$agent_country','$agent_fax','$agent_email','$s_tanotes',$b_hotel,$b_trans,$b_visa,$b_extra ,'$s_group_code')";
 $smaini = pg_query($conn, $sqlinsmain);


if (!$smaini) {
echo "An error occured during sm inserting.\n";
exit;
		}

$sequpdatemain = "update seq set sales_main=sales_main+1";
pg_query($conn, $sequpdatemain);
...
$result_hotel_s = pg_query($conn, $query_hotel_s);
...
$sqlupdatehotel = "update sales_hotels set main_sno=$main_seq,ocode='$pnr',booking_status='On Request',cus_account_code='$agent_acc_code',supp_account_code='$supp_account_code',order_date='now',option_date='$optiondate' where sales_hotels_sno=$s_sales_hotel_sno[$i]";
pg_query($conn, $sqlupdatehotel);
...
$vocsno=1;
$ac_nar ="";
$ac_nar = "CR Sales - ". $s_gtitle .". ".$s_gname." - PNR:".$pnr;
...
$ac_nar="";
$ac_nar = "CR Sales - ". $s_gtitle .". ".$s_gname." - PNR:".$pnr ." - ".$agent_c_name.",". $agent_country;
$ac_nar = substr($ac_nar,0,254);
...
$vocsno=1;
$ac_nar="";
$ac_nar = "CR Purchases - ". $s_gtitle .". ".$s_gname." - PNR:".$pnr;
$ac_nar = substr($ac_nar,0,254);
...
$vocsno++;
$ac_nar="";
$ac_nar = "CR Purchases - ". $s_gtitle .". ".$s_gname." - PNR:".$pnr ." - ".$agent_c_name.",". $agent_country;
$ac_nar = substr($ac_nar,0,254);
...
$sqlupdatetrans = "update sales_trans set main_sno=$main_seq,ocode='$pnr',booking_status='On Request',cus_account_code='$agent_acc_code',supp_account_code='$account_codet',order_date='now',option_date='$optiondate' where sales_trans_sno=$s_sales_trans_sno[$i]";
pg_query($conn, $sqlupdatetrans);
...
$query_visa_t ="select no_adults,no_child,no_infant,req_date_time,tot_net_adults,tot_sell_adults,tot_net_child,tot_sell_child,tot_net_infant,tot_sell_infant from sales_visa where sales_visa_sno=$s_sales_visa_sno[$i] ";

$result_visa_t = pg_query($conn, $query_visa_t);
...
$s_tot_amount = $tot_sell_adults + $tot_sell_child + $tot_sell_infant;
...
$sqlupdatevisa = "update sales_visa set main_sno=$main_seq,ocode='$pnr',booking_status='On Request',cus_account_code='$agent_acc_code',supp_account_code='413001',order_date='now',option_date='$optiondate' where sales_visa_sno=$s_sales_visa_sno[$i]";
pg_query($conn, $sqlupdatevisa);
...
$vocsno=1;
$ac_nar="";
$ac_nar = "CR Purchase - Nusuk Umrah - ". $s_gtitle .". ".$s_gname." - ".$pnr;
$ac_nar = substr($ac_nar,0,254);
...
$sqlinsacc4 = "insert into 	vocmast(voctype,vocno,vocsno,vocdate,acccode,narration,dbamt,cramt,pnr,invno,recon,sinvno,sinvtype,moredet) values('UP','$vocno',$vocsno,'$s_req_date_time_do_v','413001','$ac_nar',0,$gtotv_net,'$pnr',$main_seq,'f',$s_sales_visa_sno[$i],'V','$ac_md')";
pg_query($conn, $sqlinsacc4);
...
$result_oth_t = pg_query($conn, $query_oth_t);
...
$tot_amt = $tot_amt + $sell_rate;
}


$sqlupdateextra = "update sales_extra set main_sno=$main_seq,ocode='$pnr',booking_status='On Request',cus_account_code='$agent_acc_code',supp_account_code='412000',order_date='now',option_date='$optiondate' where sales_extra_sno=$s_sales_extra_sno[$i]";
pg_query($conn, $sqlupdateextra);
