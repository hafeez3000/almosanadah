...
$enc_text=$_GET["barcode"];
$passwordb = 'dorserp password';

$barcode = md5_decrypt($enc_text, $passwordb,16);



if(isset($_GET["gname"]))  $gnamewt1 = $_GET["gname"];


$passwordb = 'dorserp password';

$gnamewt = md5_decrypt($gnamewt1, $passwordb,16);
...
switch ($type)
{
        default:
                $type = 1;
        case 1:
                Barcode39 ($barcode, $width, $height, $quality, $format, $text,$gnamewt);
                break;          
}
...
//-----------------------------------------------------------------------------
function Barcode39 ($barcode, $width, $height, $quality, $format, $text, $gnamewt)
{
        switch ($format)
        {
                default:
    ...
    or die ("Cannot Initialize new GD image stream");
        ...
        $CurrentBarX1 = (int)(($width - $ActualWidth) / 2);
		...
		ImageString ($im,3, $CurrentBarX, 0, "Sohulat Al Safar Umrah Services", $Black);
		ImageString ($im,3, $CurrentBarX, 15, $gnamewt , $Black);
        ...
        if ($text != 0)
        {
                $CenterLoc = (int)(($width-1) / 2) - (int)(($FontWidth * strlen($BarcodeFull)) / 2) - 20;
        ...
        for ($i=0; $i<strlen($BarcodeFull); $i++)
        {
                ...
                for ($n=0; $n < 9; $n++)
                {
                        ...
                        {
                                case '0':
                                        ImageFilledRectangle ($im, $CurrentBarX, 30, $CurrentBarX+$NarrowBar, $height-1-$FontHeight-2, $Color);
                                        $CurrentBarX += $NarrowBar;
                                        break;


                                case '1':
                                        ImageFilledRectangle ($im, $CurrentBarX, 30, $CurrentBarX+$WideBar, $height-1-$FontHeight-2, $Color);
                                        $CurrentBarX += $WideBar;
                                        break;
                        }
                }
                ...
                $Color = $White;
                ImageFilledRectangle ($im, $CurrentBarX, 30, $CurrentBarX+$QuietBar, $height-1-$FontHeight-2, $Color);
