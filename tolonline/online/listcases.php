...
<table width="100%" cellpadding="1" cellspacing="0"><tr>
 ...
 <form name="gquot" action="<?php echo $_SERVER['PHP_SELF']; ?>"  method="post" >
  ...
  <tr> 
					...
					<tr><td> <br><br>
					<table border="1" cellpadding="1" cellspacing="0" width="98%" align="center"><tr bgcolor="#CCCCCC" style="font-weight:bold; text-align:center;"><td><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Case No.</font></td><td><font size="2" face="Verdana, Arial, Helvetica, sans-serif">User Name</font></td><td><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Case Subject</font></font
...
if (isset($_POST['action']) && $_POST['action'] == 'submitted') {
	...
	$my = $_POST['d1Year'];
	$tod = $my."-".$mm."-".$md;
	$q_main_sel ="select c.sno,c.subject,c.date_entered,c.description,c.resolution,c.status,c.priority,u.user_first_name,u.user_last_name  from cases c, users u where c.user_sno = $suser_sno and c.user_sno = u.user_sno and c.created_by=$suser_sno and c.date_entered between date '$fromd' and date '$tod' + integer '1' order by c.date_entered desc ";
	$res_main_sel = pg_query($q_main_sel);
	...
	while ($rows_main_sel = pg_fetch_array($res_main_sel)){
		...
		$user_name = $rows_main_sel["user_first_name"]." ".$rows_main_sel["user_last_name"];
		if($rows_main>0){
			...
			echo "</font></div></td>";
			echo "<td><div align=\"center\"><font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\">";
			if($s_resolution){ echo $s_resolution; } else { echo "&nbsp;"; }
			echo "</font></div></td></tr>"; 
		}
	}
} else {

	//When window loads, by default the first 10 cases should be shown.
	$query ="select c.sno,c.subject,c.date_entered,c.description,c.resolution,c.status,c.priority,u.user_first_name,u.user_last_name  from cases c, users u where c.user_sno = $suser_sno and c.user_sno = u.user_sno and c.created_by=$suser_sno order by c.date_entered desc LIMIT 10";
	$pgq = pg_query($query);
	...
	while ($pgfa = pg_fetch_array($pgq)){
		$sno = $pgfa["sno"];
		$s_sno = $pgfa["sno"];
		$s_subject = $pgfa["subject"];
		$s_date_entered = $pgfa["date_entered"];
		$s_description = $pgfa["description"];
		$s_resolution = $pgfa["resolution"];
		$s_status = $pgfa["status"];
		$s_priority = $pgfa["priority"];
		$user_name = $pgfa["user_first_name"]." ".$pgfa["user_last_name"];
		if($pgnr>0){
			echo "<tr><td><div align=\"center\"><font size=\"2\" face=\"Verdana, Arial, Helvetica, sans-serif\">";
			echo $s_sno;
			echo "</font></div></td>";
