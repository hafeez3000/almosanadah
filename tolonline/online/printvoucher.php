...
<?php
$query = "SELECT a.aname, a.pobox, a.city, a.country, a.tel1, a.tel2, a.mobile, a.email, a.wsite FROM agentsdet a, users u WHERE u.user_sno = $suser_sno AND u.account_code = a.acccode";
$pgq = pg_query($query);
...
while ($pgnr = pg_fetch_array($pgq)){
	$aname = $pgnr["aname"];
	$pobox = $pgnr["pobox"];
	$city = $pgnr["city"];
	$country = $pgnr["country"];
	$tel1 = $pgnr["tel1"];
	$tel2 = $pgnr["tel2"];
	$mobile = $pgnr["mobile"];
	$fax = $pgnr["fax"];
...
?>
<tr><td valign="top" colspan="2" align="center" style="border-bottom:1px solid #999999;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr> 
	<td align="center" ><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><?=$aname?></strong><br />
	P.O.Box: <?php if($pobox){ echo $pobox; } else { echo "&nbsp;"; } ?>, <?php if($city){ echo $city; } else { echo "&nbsp;"; } ?>, <?php if($country){ echo $country; } else { echo "&nbsp;"; } ?> 
	</font><br /><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Tel: <?php if($tel1){ echo "+".$tel1; } else { echo "&nbsp;"; } ?><?php if($tel2){ echo ", +".$tel2; } else { echo ""; } ?>, Fax: <?php if($fax){ echo "+".$fax; } else { echo "&nbsp;"; } ?>, Mobile: <?php if($mobile){ echo $mobile; } else { echo "&nbsp;"; } ?><br>Email: <?php if($email){ echo $email; } else { echo "&nbsp;"; } ?>, Web: <?php if($website){ echo $website; } else { echo "&nbsp;"; } ?></font></td>
	</tr>
	</table>
</td>
</tr>
...
$payval = "select cus_paid from sales_hotels where ocode='$s_pnr' and cus_paid!=0 ";
$vou_payval = pg_query($payval);
if (!$vou_payval) {
echo "An error occured.\n";
exit;
		}
while ($rows_vou_payval = pg_fetch_array($vou_payval)){
 $s_cus_paid[] = $rows_vou_payval["cus_paid"];
}

for($i=0; $i<count($s_cus_paid) ; $i++){
...
echo  "<font size=\"4\" face=\"Verdana, Arial, Helvetica, sans-serif\" color=\"red\"> <br>Please make payment!,<br><br> If payment already made,  kindly notify Dar Al Manasek Tourism and Umrah Services Co. Ltd. Reservation Executive </font>";
...
$payval = "select cus_paid from sales_trans where ocode='$s_pnr' and cus_paid!=0 ";
$vou_payval = pg_query($payval);
if (!$vou_payval) {
echo "An error occured.\n";
exit;
