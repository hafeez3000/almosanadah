...
$query_hotel ="select hotel_id from hotels order by hotel_id ";
...
$query_hotel_che =" SELECT relname FROM pg_class WHERE relname = 'rates$hotel_id[$i]' ";
$result_hotel_che = pg_query($query_hotel_che);
$tb_c =   pg_num_rows($result_hotel_che);

if($tb_c==1){}
else{

pg_query("create table rates$hotel_id[$i] ( rate_sno int8 NOT NULL, room_id varchar(10),   no_of_paxs int2,   rate_date date, avialibility int2,   avial_bool bool,   allotment int2,  PRIMARY KEY (rate_sno))" );
...
$result_main_del = pg_query($query_main_del);
...
pg_query($query_del_extra);


// deleteing extra roomtypes data start

echo "Processed " . $hotel_id[$i];
}   // end of hotel
