...
if(document.getElementById ('hotelsb').value == "selecth"){
 ...
 // var pval = '<? // echo $array_room_id[0] ; ?>'
   var pval = '<? if( !isset($array_room_id[0]) ||  $array_room_id[0]=="") { echo $s_hotelsb . "11"; } else { echo $array_room_id[0] ;}?>'
//  var pval = 0 ; ?>'
...
if(isset($array_room_id) && empty($array_room_id)) { echo $vt1 = $s_hotelsb . "11" + $vt; } else { echo $vt1 =  $array_room_id[0] + $vt;}
...
<form name="gquot" action="<?php echo $_SERVER[
    "PHP_SELF"
]; ?>" method="post" onSubmit="return fun2(this)">
...
<?
		for($i=0;$i<count(isset($array_room_id) ? $array_room_id : []);$i++){

 	echo "<tr>";
	echo "<td><input type=\"text\" name=\"txtid$array_room_id[$i]\" id=\"txtid$array_room_id[$i]\" size=\"10\" value=\"$array_room_id[$i]\" readonly /></td>";
...
<input type="hidden" id="it_val" name="it_val" value='<? echo count(isset($array_room_id) ? $array_room_id : []); ?>'/>
<input type="hidden" name="action" id="action" value="unsubmitted" />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
