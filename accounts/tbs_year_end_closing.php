...
?>
		  ...
		  <tr><td><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Trial Balance Sheet <br>from: &nbsp; <? echo date('D, d-M-Y', strtotime($fromd))  ." to " . date('D, d-M-Y', strtotime($tod)) . " " . $wzs . $accn_s; ?></b></font>
		   ...
		   <a href="printtbs.php?wz=<? echo $wz ?>&fd=<? echo $fromd ?>&td=<? echo $tod ?>&acc=<? echo $accn ?>&accn=<? echo $aa_name ?>" target="ptbs" onClick="window.open('', 'ptbs','width=760,height=450,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()"><img src="../images/print_icon.gif" width="16" height="16"></a> &nbsp;<a href="export2excel.php" target="ptbs"><img src="../images/excel.png" title="Export to Excel" alt="Export to Excel" width="16" height="16"></a>
...
if($z_check==1){  // if zero check
	  ...
	  echo "</tr>";	
		  ...
		  if($ASS==$atype[$i] || $EXP==$atype[$i]){
						$eycb = $fbaldb - $fbalcr;
						$eycac = $aaccode[$i];
		$assert_q = "UPDATE accmast2009 set op_bal= $eycb where acccode = $eycac " ;
		
		pg_query($conn, $assert_q);
