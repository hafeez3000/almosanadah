...
<table name="hmenutable" id="hmenutable" width="100%" border="0" cellpadding="0" cellspacing="0" style="border-bottom: 1px solid #999999 ;border-left: 3px solid #006600;border-right: 3px solid #006600"><tr>
    <td bgcolor="#CCCCCC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">&nbsp;You 
      are here: Home &raquo; Queries &raquo; CashFlow</font></td>
  </tr></table>
...
$accn_s="";
$accn_qt="";
if($accn=="payments"){$accn_qt="where CAST(acccode AS INTEGER) between 400000 and 500000 " ; $accn_s="<br>Hotel & Supplier Payments"; $wzs="<br>With Credit Balance Only" ;} else { 
$accn_qt="where CAST(acccode AS INTEGER) between 150000 and 399999" ; $accn_s="<br>Agents Receivables"; $wzs="<br>With Debit Balance Only" ;} 


?>
		  <table width="100%" border="0" cellpadding="1" cellspacing="0">
		  <tr><td><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Cash Flow Sheet <br>from: &nbsp; <? echo date('D, d-M-Y', strtotime($fromd))  ." to " . date('D, d-M-Y', strtotime($tod)) . " " . $accn_s . $wzs; ?></b></font>
		  &nbsp;&nbsp;
		  
		   <a href="printcashflow.php?wz=<? echo $wz ?>&fd=<? echo $fromd ?>&td=<? echo $tod ?>&acc=<? echo $accn ?>&accn=<? echo $aa_name ?>" target="ptbs" onClick="window.open('', 'ptbs','width=760,height=450,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()"><img src="../images/print_icon.gif" width="16" height="16"></a>

		&nbsp;&nbsp;

		<a href="printcashflowactual.php?wz=<? echo $wz ?>&fd=<? echo $fromd ?>&td=<? echo $tod ?>&acc=<? echo $accn ?>&accn=<? echo $aa_name ?>" target="ptbs" onClick="window.open('', 'ptbs','width=760,height=450,menubar=yes,scrollbars=yes, top='+wint+',left='+winl+' ').focus()"><img src="../images/actual.jpg" ></a>
...
$op_query = "select  SUM(dbamt) as totdb, SUM(cramt) as totcr from vocmast where cast(acccode as integer)=$aaccode[$i] and vocdate between '$start_period' and date '$fromd' - interval '1 day' ";
...
$totquery = "select  SUM(dbamt) as totdb, SUM(cramt) as totcr from vocmast where cast(acccode as integer)= $aaccode[$i] and vocdate between '$fromd' and '$tod' ";
